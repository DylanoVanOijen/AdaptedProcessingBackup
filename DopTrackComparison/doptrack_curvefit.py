...
LOG_FILE = '/home/doptrackbox/DTB_Recordings/locally_processed/fitted_doptrack_files.log'
EXTRACTION_LOC = '/home/doptrackbox/DTB_Recordings/locally_processed/Processed/DopTrack/'
...
def find_yml(file, name, year):
    ...
    try:
        ...
        yml_path = DopTrackLoc + "products/L0/" + yml_subdir
...
def store_extracted_signal(time, freq, power, noise, snr, label, mask_width): 
    ...
    print("Storing in", os.path.join(EXTRACTION_LOC, f"ExtractedSignal/{label}.txt"))
...
if __name__ == "__main__":
    ...
    try:    
        ...
        TUNE = yml['recording']['tuning_frequency']
        ...
        REC_START = yml['recording']['time_start'].replace(tzinfo=timezone.utc)        
        ...
        #ax.hist(flattened_freqs, bins=freq_bins)
        #ax.hlines(occurance_treshold*n_samp, min(freq_bins), max(freq_bins))
        #ax.grid()
        #fig_1.set_size_inches(19, 10)
        #fig.tight_layout()
        #os.makedirs(PLOT_LOC+"FreqHist", exist_ok=True)
        #fig.savefig(os.path.join(PLOT_LOC, f"FreqHist/freq_hist_{FILE}.png"))
        #plt.close(fig)
        #gc.collect()

        #peaks = np.argsort(freq_hist)[::-1]  # sort bins by count, descending
        #dominant_bins = peaks[:5]       # top 5 frequent frequencies
        #print(dominant_bins)
        ...
        #values, counts = np.unique(freq_data, return_counts=True)
        #frequent_values = values[counts > occurance_treshold*len(freq_data)]
        
        """ while len(dominant_freqs) > 0:
            # Instead of taking max snr, should just throw all frequencies on one heap, then make mask based on that
            full_time_data, full_freq_data, full_power_data, full_noise_data, full_snr_data = frequency_filter(full_time_data, full_freq_data, full_power_data, full_noise_data, full_snr_data, freq_mask)
            freq_data, snr_data, power_data = get_max_snr_frequencies(full_freq_data, full_snr_data, full_power_data)

            freq_hist, bin_edges = np.histogram(freq_data, bins = freq_bins, density=True)
            bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
            mask = freq_hist > occurance_treshold
            dominant_freqs = bin_centers[mask]
            freq_mask.extend(dominant_freqs) """

            #values, counts = np.unique(freq_data, return_counts=True)
            #frequent_values = values[counts > occurance_treshold*len(freq_data)]
            #freq_mask.extend(frequent_values)

        initial_guess = compute_initial_guess(full_time_data, center_frequency, name)
        DT_initial_guess = None

        if "FUNcube" in name:
            print("Retrieving initial guess")
